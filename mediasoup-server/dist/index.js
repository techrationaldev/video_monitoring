...
wss.on("connection", (ws, req) => {
    ...
    ws.on("message", async (message) => {
        try {
            ...
            console.log(`[SERVER] Received action: ${action} | Room: ${roomId} | Client: ${clientId || "N/A"}`);
            if (!roomId) {
            ...
            if (action === "join-as-admin") {
                console.log(`[SERVER] Admin joined dashboard. Total admins: ${adminClients.size + 1}`);
                adminClients.add(ws);
                ...
                if (room.getProducers().length === 1 ||
                    ...
                    setTimeout(async () => {
                        try {
                            ...
                            console.log(`[SERVER] Checking auto-recording trigger: Transports=${room.recordingTransports.size}, Producers=${currentProducers.length}, IsRecording=${room.isRecording}`);
                            if (room.recordingTransports.size === 0 &&
                                currentProducers.length > 0 &&
                                !room.isRecording) {
                                room.isRecording = true;
                                console.log(`[SERVER] Triggering auto-recording for room ${roomId}`);
                                await axios.post(`${config.recordingServiceUrl}/recording/start`, {
                                    roomId,
                                }, {
                                    headers: {
            ...
            if (action === "stop-stream") {
                ...
                if (!roomId ||
                    ...
                    console.error("[SERVER] Missing or invalid roomId/clientId for stop-stream");
                    return;
                }
                ...
                if (room) {
                    ...
                    if (room.isRecording) {
                        console.log(`[SERVER] Stopping recording for room ${roomId} (User Action)`);
                        axios
                            .post(`${config.recordingServiceUrl}/recording/stop`, {
                            roomId: roomId,
                        }, {
                            headers: {
                                Authorization: `Bearer ${config.internalApiSecret}`,
                            ...
                            .then((response) => {
                            console.log(`[SERVER] Recording stopped successfully for room ${roomId}`);
                            if (response.data && response.data.url) {
                                console.log(`[SERVER] Recording URL: ${response.data.url}`);
                            }
                        })
                            .catch((err) => console.error(`[SERVER] Failed to stop recording: ${err.message}`));
                    ...
                    axios
                        ...
                        .catch((err) => console.error(`[SERVER] Failed to notify backend: ${err.message}`));
        ...
        if (currentRoomId && currentClientId) {
            ...
            const timeout = setTimeout(() => {
                ...
                if (roomForStop && roomForStop.isRecording) {
                    console.log(`[SERVER] Triggering stop-recording for room ${currentRoomId}`);
                    axios
                        .post(`${config.recordingServiceUrl}/recording/stop`, {
                        roomId: currentRoomId,
                    }, {
                        ...
                        .catch((err) => {
                        console.error(`[SERVER] Failed to stop recording: ${err.message}`);
                    });
