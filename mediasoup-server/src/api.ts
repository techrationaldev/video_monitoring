...
app.post('/create-recording-transport', checkAuth, async (req, res) => {
    ...
    try {
        ...
        const transports = await room.createRecordingTransportTuple(recordingIp, audioPort, videoPort);
        ...
        const audioCodec = audioProducer && audioProducer.appData.rtpParameters ? {
             payloadType: audioProducer.appData.rtpParameters.codecs[0].payloadType,
             mimeType: audioProducer.appData.rtpParameters.codecs[0].mimeType,
             clockRate: audioProducer.appData.rtpParameters.codecs[0].clockRate,
             channels: audioProducer.appData.rtpParameters.codecs[0].channels
        } : null;

        const videoCodec = videoProducer && videoProducer.appData.rtpParameters ? {
             payloadType: videoProducer.appData.rtpParameters.codecs[0].payloadType,
             mimeType: videoProducer.appData.rtpParameters.codecs[0].mimeType,
             clockRate: videoProducer.appData.rtpParameters.codecs[0].clockRate
        } : null;

        const sdp = generateSDP(recordingIp, audioPort, videoPort, audioCodec, videoCodec);
        ...
        const mainTransport = transports.length > 0 ? transports[0].transport.id : 'none';
